...
import { makeTrafficLight, makeTrafficLightMesh, makeStopSign, makePlanter, makeAwning, makeBikeRack, makeFountain, makeKiosk, makeSignBand } from '../src/world/objects'
...
describe('street objects', () => {
  ...
  it('bikeRack / fountain / kiosk / signBand geo non-empty & deterministic', () => {
    nonEmptyDet(() => makeBikeRack()); nonEmptyDet(() => makeFountain())
    nonEmptyDet(() => makeKiosk()); nonEmptyDet(() => makeSignBand({w:3.0}))
  })
})
