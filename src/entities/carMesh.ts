...
export function makeCarBody(opts: CarBodyOpts): THREE.Group {
  ...
  // =======================================================================
  const body = new THREE.Mesh(boxGeo(1.78, 0.5, 3.5), paint);
  ...
  const hood = new THREE.Mesh(boxGeo(1.7, 0.12, 0.95), paint);
  hood.position.set(0, 0.28, 1.2); hood.castShadow = true;
  g.add(hood);

  // Trunk / hatch: lower deck at the rear (-z). For 'hatch' it's short & stubby.
  const trunk = new THREE.Mesh(boxGeo(1.7, 0.12, rearTrunkLen), paint);
  ...
  // Sloped REAR window.
  const rearGlass = new THREE.Mesh(boxGeo(cabinW - 0.06, 0.5, 0.07), glass);
  ...
  for (const x of [-(cabinW / 2 - 0.02), cabinW / 2 - 0.02]) {
    // A-pillar (raked)
    const a = new THREE.Mesh(boxGeo(0.07, cabinH + 0.08, 0.08), paint);
    a.position.set(x, cabinBaseY + cabinH / 2, pillarFrontZ);
    a.rotation.x = -0.42;
    g.add(a);
    // B-pillar (vertical, between doors)
    const b = new THREE.Mesh(boxGeo(0.06, cabinH, 0.07), paintDark);
    b.position.set(x, cabinBaseY + cabinH / 2, pillarMidZ);
    g.add(b);
    // C-pillar (rear, raked)
    const c = new THREE.Mesh(boxGeo(0.07, cabinH + 0.06, 0.08), paint);
  ...
  // =======================================================================
  const rearZ = -1.74;
  const rearBumper = new THREE.Mesh(boxGeo(1.78, 0.22, 0.16), trim);
  rearBumper.position.set(0, -0.16, rearZ - 0.02); g.add(rearBumper);

  for (const x of [-0.62, 0.62]) {
    const tl = new THREE.Mesh(boxGeo(0.32, 0.2, 0.07), taillight);
    tl.position.set(x, 0.08, rearZ - 0.02); g.add(tl);
  }
  const rearPlate = new THREE.Mesh(boxGeo(0.42, 0.12, 0.04), plate);
  ...
  // =======================================================================
  for (const x of [-1, 1]) {
    const stalk = new THREE.Mesh(boxGeo(0.12, 0.04, 0.04), trim);
    stalk.position.set(x * 0.92, 0.3, pillarFrontZ - 0.02); g.add(stalk);
    const cap = new THREE.Mesh(boxGeo(0.06, 0.12, 0.16), paint);
  ...
  if (opts.withWheels) {
    ...
    for (const [x, z] of positions) {
      ...
      const tire = new THREE.Mesh(cylGeo(0.35, 0.35, 0.3, 16), tireMat);
      tire.rotation.z = Math.PI / 2;
      tire.castShadow = true; wheel.add(tire);

      // Lighter hubcap on the OUTER face so wheels aren't plain black pucks.
      const hub = new THREE.Mesh(cylGeo(0.16, 0.16, 0.06, 12), hubMat);
      ...
      const arch = new THREE.Mesh(boxGeo(0.1, 0.18, 0.74), trim);
