...
function makeFIDSTexture(rows: number): THREE.CanvasTexture {
  ...
  for (let i = 0; i < clampedRows; i++) {
    ...
    ctx.fillText(fd.status, 800, textY);
...
defineObject("flightBoard", {
  params: { w: 6, h: 3, rows: 8 } as FlightBoardParams,
  build(p: FlightBoardParams): ObjectResult {
    const { w, h, rows } = p;
    const depth = 0.2;
    const mountH = 0.5;  // vertical mount post below board

    const parts: THREE.BufferGeometry[] = [];
    const group = new THREE.Group();

    // ── Board body (dark box) ───────────────────────────────────────────────
    // The board is offset upward by the mount height; base y=0 is the mount
    const boardBotY = mountH;
